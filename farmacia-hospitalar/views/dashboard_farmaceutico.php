<?php
/**
 * views/dashboard_farmaceutico.php
 * Painel do Farmacêutico: fila de requisições da sua farmácia, criação de
 * pedidos, perfil, minhas atividades e gestão de equipe (cadastro de usuários).
 */
define('PERFIL_EXIGIDO', ['farmaceutico', 'admin']);
require_once __DIR__ . '/../includes/auth_check.php';

$nomeUsuario = htmlspecialchars($_SESSION['nome']);
$crfUsuario  = htmlspecialchars($_SESSION['crf'] ?? '');
...
<head>
    ...
    <title>Farmácia Hospitalar - Painel do Farmacêutico</title>
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col antialiased">
            ...
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-teal-100">Unidade: <?= $siglaFarmacia ?> &middot; Farmacêutico(a)</p>
                    </div>
                </div>
                ...
                <nav class="flex space-x-2 items-center">
                    ...
                    <button id="nav-equipe" onclick="trocarAba('equipe')" class="px-3 py-2 rounded-lg text-sm font-semibold transition-all duration-200 text-teal-100 hover:text-white hover:bg-white/10 flex items-center space-x-2">
                    ...
                    <span class="text-sm text-teal-100 hidden sm:inline">
                        <?= $nomeUsuario ?><?= $crfUsuario ? " (CRF $crfUsuario)" : '' ?>
                    </span>
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div id="view-perfil" class="hidden max-w-2xl mx-auto space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-100">
                        <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wider block">CRF</span>
                        <span id="perfil-crf" class="text-sm font-semibold text-slate-700 block mt-1">-</span>
                    </div>
                </div>
        ...
        <!-- ABA: EQUIPE (exclusiva farmacêutico/admin) -->
        <div id="view-equipe" class="hidden space-y-6">
            <div class="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
                <div class="bg-gradient-to-r from-teal-700 to-emerald-800 text-white p-6">
                    <div class="flex items-center space-x-3">
                        <i class="fa-solid fa-user-plus text-3xl"></i>
                        <div>
                            <h2 class="text-xl font-bold">Cadastrar Novo Perfil</h2>
                            <p class="text-teal-100/90 text-xs mt-1">A senha padrão é definida automaticamente; a pessoa pode trocá-la depois pelo próprio Perfil.</p>
                        </div>
                    </div>
                </div>

                <form id="form-cadastrar-usuario" class="p-6 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">Nome Completo</label>
                            <input type="text" id="novo-usuario-nome" required class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">E-mail</label>
                            <input type="email" id="novo-usuario-email" required class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">Login (nome de usuário)</label>
                            <input type="text" id="novo-usuario-login" required class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">CRF (opcional)</label>
                            <input type="text" id="novo-usuario-crf" class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">Perfil de Acesso</label>
                            <select id="novo-usuario-perfil" required class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm bg-white">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-500 tracking-wider mb-2">Farmácia de Atuação</label>
                        <select id="novo-usuario-farmacia" required class="w-full px-3 py-2.5 rounded-lg border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm bg-white">
                            <?php foreach ($farmacias as $f): ?>
                                <option value="<?= $f['id_farmacia'] ?>"><?= htmlspecialchars($f['sigla']) ?> - <?= htmlspecialchars($f['nome_completo']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase bg-slate-50">
                                ...
                                <th class="px-6 py-3.5">Farmácia</th>
                                <th class="px-6 py-3.5">Status</th>
                            </tr>
                        </thead>
                        <tbody id="equipe-list" class="divide-y divide-slate-100 text-sm"></tbody>
                    </table>
                </div>
            </div>
        </div>
